...
# Imax = 40 #maximum number of inpatients
...
##############BELOW HERE ALL SHOULD BE THE SAME##############
...
for nr_pat in range(max_nr_i+1):
    pos_arrivals_c = []
    pos_arrivals_cdf_c = []
    pos_arrivals_prob_c = []
    prob_nrr = []
    ...
    for c in range(C):
        prob_tot = []
        prob_tot_x = [[]]
        for i in range(max(0, min(10, max_nr_i-nr_pat))+1):
            # prob_x.append(np.multiply(Q[n],i))
...
# print(pos_arrivals)
    ...
    for t in range(timeinterval):
        ...
        # pos_ar_prob_t.append(pos_arrivals_prob)
...
class Outpatient():
    def __init__(self, name):
        self.name = name
        # self.nrMDO = st.poisson.rvs(mu_o)
        # self.nrMDO = st.hypergeom.rvs(100,22, 7, loc=0, random_state=None)
        ...
        # self.fullTreatmentPlan = np.random.choice(range(1, len(P)+1),size = self.nrMDO+1, p= P_dist)
    ...
    def getAdmitted(self):
        if self.waitingList:
            ...
            self.currentMDO = 0
            self.L = T_start_o
            # print(str(self.name)+' is admitted')
        else: 
    ...
    def totalDemand(self):
        ...
        demand =[np.zeros((C))]
        # demand = [[0] for c in range(C)]
        for pp in range(ll):
            if pp == ll-1:
                # print( [P[self.fullTreatmentPlan[pp]-1]]*8)
                # print(demand)
                # print([P[self.fullTreatmentPlan[pp]-1]]*(T_start_o+1))
                demand =np.concatenate((demand, [P[self.fullTreatmentPlan[pp]-1]]*(T_start_o+1)), axis = 0)
                # demand.append([P[self.fullTreatmentPlan[pp]-1]]*8)
            else:
                # print(demand)
                # print([P[self.fullTreatmentPlan[pp]-1]]*(T_start_o-T_MDO_o+1))
...
class Inpatient():
    def __init__(self, name):
        self.name = name
        # self.nrMDO = st.poisson.rvs(mu_i)
        # self.nrMDO = st.hypergeom.rvs(100,14, 7)
        ...
        # self.fullTreatmentPlan = rng.choice(range(1, len(Q)+1), size = self.nrMDO+1, p = Q_dist)
        self.stay =0
        self.L = T_start_i
        self.currentPlan = self.fullTreatmentPlan[0]
...
def makeInitialization_random(T =100):
    ## estimated guess theta beter than myopic
...
def getDemandDecision_I_withL(inpatient_list):
    demand = np.zeros((len(L_i), max_cont_i ,C))
    for t in range(len(L_i)):
        for pat in inpatient_list:
            if pat.L-t == T_MDO_i:
                dd = pat.returnDemandCURRENT()
                for c in range(C):
                    demand[t][pat.currentMDO][c] += dd[c]
    return demand

def getDemandDecision_O_withL(outpatient_list):
    demand = np.zeros((len(L_o), max_cont_o ,C))
    for t in range(len(L_o)):
...
def PARA_demandDecision_I_PREDICTION(inpatient_list, timeinterval, theta):
    summ_t = np.zeros((timeinterval, C))      
    for t in range(timeinterval-T_MDO_i-1):      
        for pat in inpatient_list:
            if pat.L == T_MDO_i+t:
                summ_t[t+T_MDO_i+1:]+= theta*np.multiply(Q[pat.currentPlan-1],probs_i[pat.currentMDO])
    return summ_t

def PARA_demandDecision_O_PREDICTION(outpatient_list, timeinterval, theta):
    summ_t = [[0]*C]*(T_MDO_o+1)      
    for t in range(timeinterval-T_MDO_o-1):
        summ = [0]*C
        for pat in outpatient_list:
            if pat.L == T_MDO_o+t:
                summ+= theta*np.multiply(P[pat.currentPlan-1],probs_o[pat.currentMDO])
        summ_t.append(summ)
    return summ_t

def PARA_demandDecision_I_with_L(inpatient_list, timeinterval, theta):
    demand = np.zeros((timeinterval, C))
    xx = getDemandDecision_I_withL(inpatient_list)
    for tt in range(len(xx)):
        for t in range(T_MDO_i+1, min(timeinterval-tt,T_MDO_i+len(L_i))):
            for i in range(max_cont_i-1):
                for c in range(C):
                    demand[tt+t][c] += theta[i]*xx[tt][i][c] 
    return demand

def PARA_demandDecision_O_with_L(outpatient_list, timeinterval, theta):
    demand = np.zeros((timeinterval, C))
    xx = getDemandDecision_O_withL(outpatient_list)
    for tt in range(len(xx)):
        for t in range(T_MDO_o+1, min(timeinterval-tt,T_MDO_o+len(L_o))):
            for i in range(max_cont_o-1):
                for c in range(C):
                    demand[tt+t][c] += theta[i]*xx[tt][i][c] 
    return demand
...
def PARA_decoutp_NEWS(patient_waiting_list, inpatient_list, outpatient_list, inp_trj, outp_trj, timeinterval, alpha):
    ...
    for i in range(timeinterval):
        # print( np.sum(thetas[:i+1], axis = 0)) 
        for c in range(C):
    ...
    for t in range(timeinterval-1):
        ...
        for c in range(C):
            ...
            if len(thetas) == 0:     
                ...
                # print(current_demands[t][c])
                # print(thetas)
                # print(np.sum(thetas, axis = 0)[c])
            ...
            # print(total_action_demand)
            # print(((total_urgency/total_action_demand[c])+C_cost[c]*(1-pr)) )
            if total_action_demand[c] == 0 or ((total_urgency/total_action_demand[c])+C_cost[c]*(1-pr)) == 0:
                # theta1 = 0.5
                # theta2 = 0.5
                theta1 = C_cost[c]*(1-pr)/(C_cost[c]+C_cost[c]*(1-pr))
...
def PARA_newDemand_inp_NEWS(patient_waiting_list, inpatient_list, outpatient_list, outp_trj, timeinterval, alpha):
    ...
    for pat in inpatient_list:
        for t in range(timeinterval):
            if pat.L  >= t:
                nr_pat[t]+=1
    # pos_ar_t,pos_ar_cdf_t = getPosArrivals(inpatient_list, timeinterval)
    
    extra_pat = 0
    for t in range(timeinterval-1):
        # print('________')
        # print(t)
        # print(nr_pat[t]+extra_pat)
        # print(extra_pat)
        total_urgency = 0 
        total_action_demand = np.zeros((C))
        # pos_arrivals = pos_arrivals_nrpat[nr_pat[t]+extra_pat]
        # pos_arrivals_cdf = pos_arrivals_cdf_nrpat[nr_pat[t]+extra_pat]
        for pat in patient_waiting_list:
            total_urgency += U_cost[pat.urgency]
            total_action_demand[:] += P[pat.fullTreatmentPlan[0]-1]
        if t>0:
        ...
        for c in range(C):
            ...
            if current_demand+total_action_demand[c]+theta1_f < f[c]:
                ...
                # print('2')
                # # print([nr_pat[t]+extra_pat])
                # # print(prob_nr_cdf_tot[nr_pat[t]+extra_pat])
                # print([nr_tot[nr_pat[t]+extra_pat][i] for i in range(len(prob_nr_cdf_tot[nr_pat[t]+extra_pat])) if prob_nr_cdf_tot[nr_pat[t]+extra_pat][i]>= theta2][0])
                extra_pat_c += [nr_tot[nr_pat[t]+extra_pat][i] for i in range(len(prob_nr_cdf_tot[nr_pat[t]+extra_pat])) if prob_nr_cdf_tot[nr_pat[t]+extra_pat][i]>= theta2][0]
            else: 
                theta_c.append(theta1_f)
                # print('1')
                # print([nr_tot[nr_pat[t]+extra_pat][i] for i in range(len(prob_nr_cdf_tot[nr_pat[t]+extra_pat])) if prob_nr_cdf_tot[nr_pat[t]+extra_pat][i]>= theta1][0])
...
def TUNE_demandDecision_O_NEWS2(outpatient_list, inpatient_list, patient_waiting_list, trj_inp, trj_outp, timeinterval, theta, alpha):
    return PARA_decoutp_NEWS2(patient_waiting_list, inpatient_list, outpatient_list, trj_inp, trj_outp, timeinterval, alpha)

def TUNE_demandDecision_I_PRED(outpatient_list, inpatient_list, patient_waiting_list, trj_inp, trj_outp, timeinterval, theta_I, alpha):
    demand_outp = getRealDemandOutpatient(outpatient_list, timeinterval)

    demand_inp_known = getDemandList_I(inpatient_list, timeinterval)
    demand_inp_new = getDemandNew_Inpatient(inpatient_list, trj_inp, timeinterval)
    demand_inp_PARA = PARA_demandDecision_I_PREDICTION(inpatient_list, timeinterval, theta_I)

    return [[demand_outp[tt][c] + demand_inp_known[tt][c] + demand_inp_new[tt][c] + demand_inp_PARA[tt][c] for c in range(C)]for tt in range(timeinterval)]
    # return [[ demand_inp_known[tt][c] + demand_inp_new[tt][c] + demand_inp_PARA[tt][c] for c in range(C)]for tt in range(timeinterval)] 

def TUNE_demandDecision_O_PRED(outpatient_list, inpatient_list, patient_waiting_list, trj_inp, trj_outp, timeinterval, theta_I, alpha ):
    demand_inp = getRealDemandInpatient(inpatient_list, trj_inp, timeinterval)
    demand_outp_known = getDemandList_O(outpatient_list, timeinterval)
    demand_outp_PARA = PARA_demandDecision_O_PREDICTION(outpatient_list, timeinterval, theta_I)

    return [[demand_inp[tt][c] + demand_outp_known[tt][c] + demand_outp_PARA[tt][c]  for c in range(C)]for tt in range(timeinterval)]

def TUNE_demandDecision_I_with_L(outpatient_list, inpatient_list, patient_waiting_list, trj_inp, trj_outp, timeinterval, theta_I_with_L, alpha ):
    demand_outp = getRealDemandOutpatient(outpatient_list, timeinterval)

    demand_inp_known = getDemandList_I(inpatient_list, timeinterval)
    demand_inp_new = getDemandNew_Inpatient(inpatient_list, trj_inp, timeinterval)
    demand_inp_PARA = PARA_demandDecision_I_with_L(inpatient_list, timeinterval, theta_I_with_L)

    return [[demand_outp[tt][c] + demand_inp_known[tt][c] + demand_inp_new[tt][c] + demand_inp_PARA[tt][c] for c in range(C)]for tt in range(timeinterval)]
    # return [[ demand_inp_known[tt][c] + demand_inp_new[tt][c] + demand_inp_PARA[tt][c] for c in range(C)]for tt in range(timeinterval)]

def TUNE_demandDecision_O_with_L(outpatient_list, inpatient_list, patient_waiting_list, trj_inp, trj_outp, timeinterval, theta_O_with_L, alpha ):
    demand_inp = getRealDemandInpatient(inpatient_list, trj_inp, timeinterval)
    demand_outp_known = getDemandList_O(outpatient_list, timeinterval)
    demand_outp_PARA = PARA_demandDecision_O_with_L(outpatient_list, timeinterval, theta_O_with_L)
...
def TUNE_alles_NEWS(outpatient_list, inpatient_list, patient_waiting_list, inpatients_trajectory, outpatients_trajectory, timeinterval, theta, alpha):
    ...
    # list1 = [[pos_opt_total_I[t], pos_opt_prob_totaal_I[t]] for t in range(timeinterval)]
    # list2 = [[pos_opt_total_O[t], pos_opt_prob_totaal_O[t]] for t in range(timeinterval)]
    # print(pos_opt_total_I)
    ...
    for t in range(0, timeinterval-1):
        # print('__________'+str(t)+'___________')
        theta_c = []
        extra_pat_c = 0
        for c in range(C):
            # print(pos_opt_total_I[t])
            # print(pos_opt_total_I[t][c])
            
            # if t > T_MDO_i: 
            #     if len(pos_opt_total_I[t][c])>1 and len(pos_opt_total_O[t][c])>1:
            #         results = getProbCombination_check([pos_opt_total_I[t], pos_opt_prob_totaal_I[t]], [pos_opt_total_O[t], pos_opt_prob_totaal_O[t]])
                    
            #     elif len(pos_opt_total_I[t])>1:
...
def determineAction(demand, patient_waiting_list, outpatients_trajectory, timeinterval, action = True):
    ...
    with gp.Env(empty=True) as env:
        ...
        # env.setParam("WLSAccessID", '6a4d925b-5c61-4f48-a88f-2cab8ab729dc')
        # env.setParam("WLSSECRET", '0936fc4b-3762-4e07-9a7c-92e002d91056')
        # env.setParam("LICENSEID", 2526120)
        env.start()
        with gp.Model(env=env) as m:
            ...
            if action: 
                ...
                actions = []  
                # obj_t = np.zeros((T))
                # obj_names = [[] for i in range(T)]
                # obj_names_2 = [[i] for i in range(T)]
...
def determineTotalCost_actionsList(patient_waiting_list, outpatient_list, inpatient_list, outpatients_trajectory, inpatients_trajectory, timerange, action_list, totalCostOnly = False):
    ...
    for t in range(timerange):
        # print(t)
        # print('H:\n')
        # print(makeHMatrix(patient_waiting_list))
        # print(action_list[t])
        ...
        # print([i<0 for u in range(U) for i in post_decision_H[u] ])
        # if  any([i<0 for u in range(U) for i in post_decision_H[u] ]):
        #     raise ValueError
        ...
        # for pat in outpatient_list:
        #     currentNames.append([pat.name, pat.currentPlan])
        # demandArrayNames.append(currentNames)
